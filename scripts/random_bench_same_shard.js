...
function buildSameShardTransferBody({
  minAccountId,
  maxAccountId,
  amount,
  shardCount,
  ...
  const targetShardId = getShardId(fromId, shardCount);

  let toId = randomInt(minAccountId, maxAccountId);

  while (
    toId === fromId ||
    getShardId(toId, shardCount) !== targetShardId
  ) {
...
async function main() {
  ...
  console.log('=== Same-Shard Random Bench Config ===');
  console.log(`url           : ${url}`);
  ...
  const instance = autocannon({
    ...
    setupClient(client) {
      client.setBody(buildSameShardTransferBody({
        minAccountId,
        maxAccountId,
        amount,
        shardCount,
...
main().catch(err => {
  console.error('[random_bench_same_shard] failed:', err.message);
  process.exit(1);
});
