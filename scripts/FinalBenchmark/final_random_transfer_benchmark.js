...
function safeRedisMGet() {
  try {
    const output = execSync(
      'redis-cli MGET bench:transfer:sameShard bench:transfer:crossShard bench:transfer:success bench:transfer:failed',
      { encoding: 'utf8' }
    );

    return output.trim();
  } catch (err) {
    return `[redis-cli MGET failed] ${err.message}`;
  }
}

function safeReadyQueueLength() {
  try {
    const output = execSync(
      'redis-cli LLEN transfer:queue:ready:fromIds',
      { encoding: 'utf8' }
    );

    return output.trim();
  } catch (err) {
    return `[redis-cli LLEN failed] ${err.message}`;
  }
}

async function main() {
  const url = getArg('url', 'http://127.0.0.1:7010/transfers');
  const connections = toPositiveInt(getArg('connections', '100'), 100);
  const duration = toPositiveInt(getArg('duration', '60'), 60);
  ...
  console.log('========================================');
  console.log('Final Random Transfer Benchmark');
  console.log('========================================');
  console.log(`url           : ${url}`);
  ...
  instance.on('done', result => {
    ...
    console.log('\n=== Redis Counters ===');
    console.log(safeRedisMGet());

    console.log('\n=== Ready Queue Length ===');
    console.log(safeReadyQueueLength());
  });
}

main().catch(err => {
  console.error('[final_random_transfer_benchmark] failed:', err.message);
  process.exit(1);
});
